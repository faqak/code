...
int main() {
  ...
  struct Li_Chao {
    using linear = std::pair<i64, i64>;
    struct Node {
      int l, r;
      bool contain;
      linear best;
      Node() { contain = 0, best = {0ll, LLONG_MAX}; }
    };
    std::vector<Node> t;
    inline i64 Get(const linear &l, int x) { return l.first * x + l.second; }
    ...
    void Add(int o, const linear &cur) {
      int l = t[o].l, r = t[o].r;
      auto ori = t[o].best;
      if (!t[o].contain) return t[o].contain = 1, t[o].best = cur, void();
      if (Get(cur, l) <= Get(ori, l) && Get(cur, r) <= Get(ori, r)) return t[o].contain = 1, t[o].best = cur, void();
  ...
  std::vector<i64> f(n), pre(n);
  for (int &t : h) std::cin >> t, mx = std::max(mx, t);
  Li_Chao solve(mx);
  for (int &t : w) std::cin >> t;
  for (i64 &t : f) t = 0;
  pre[0] = w[0];
